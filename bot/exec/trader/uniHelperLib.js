...
// CUSTOM
// Took out RouterV01 json, because it did not work correctly. RouterV02.json not present in this directory, so imported the uniAbi file
  ...
  const safetyAmount = inputAmount
    ? expectedAmount.mul(
        ethers.BigNumber.from(1)
          .sub(ethers.BigNumber.from(maxSlippage).div(10000))
      )
    : expectedAmount.mul(
        ethers.BigNumber.from(1)
          .add(ethers.BigNumber.from(maxSlippage).div(10000))
      );
...
async function swapTokens({
  ...
  // NOTE: removing these lines will throw an error around unpredictable gas "Error: 
  // cannot estimate gas; transaction may fail or may require manual gas limit (error"
  // Not sure why these lines prevent this error, but they cannot be removed without further debugging
